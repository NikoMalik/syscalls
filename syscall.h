...
#define munmap(addr, len) \
    (syscall2(SYS_munmap, (long)(addr), (long)(len)) == 0)

#define mprotect(addr, len, prot) \
    (syscall3(SYS_mprotect, (long)(addr), (long)(len), (long)(prot)) == 0)
...
/* ---------------- resource usage ---------------- */
#define getrusage_raw(who, usage) \
...
__attribute__((constructor)) static void mi_init_auxv(void) {
    // get auxv from stack
    // in Linux: stack when start: argc → argv → envp → auxv
    register unsigned long sp asm("rsp");
    unsigned long *ptr = (unsigned long *)sp;
    unsigned long argc = ptr[0];
    ptr += argc + 2;
    while (*ptr != 0)
        ptr++;
    ptr++;
    mi_auxv = (Elf64_auxv_t *)ptr;
}
...
__attribute__((always_inline)) static void *mi_vdso_lookup(const char *name) {
